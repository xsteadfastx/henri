...
if [ "${DEPS}" = "True" ]; then
        # pkg_resources
        ln -s /henri/submodules/pycopy-lib/pkg_resources/pkg_resources.py /henri/submodules/pycopy/ports/"${PORT}"/modules/pkg_resources.py

        # uasyncio
        ln -s /henri/submodules/pycopy-lib/uasyncio/uasyncio /henri/submodules/pycopy/ports/"${PORT}"/modules/uasyncio
        ln -s /henri/submodules/pycopy-lib/uasyncio.core/uasyncio/core.py /henri/submodules/pycopy/ports/"${PORT}"/modules/uasyncio/core.py

        # picoweb
        ln -s /henri/submodules/picoweb/picoweb /henri/submodules/pycopy/ports/"${PORT}"/modules/picoweb

        # ulogging
        ln -s /henri/submodules/pycopy-lib/ulogging/ulogging.py /henri/submodules/pycopy/ports/"${PORT}"/modules/ulogging.py

        # random
        ln -s /henri/submodules/pycopy-lib/random/random.py /henri/submodules/pycopy/ports/"${PORT}"/modules/random.py
fi
## henri
if [ "${HENRI}" = "True" ]; then
